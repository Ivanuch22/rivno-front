...
interface IJobQuestionareForm {
  ...
  user: IUser | null;
  handleSetSecondFormComplete: () => void;
  isSecondFormComplete: boolean;
}
...
const JobQuestionnaireFrom: React.FC<IJobQuestionareForm> = ({
  ...
  userByIdRefetch,
  user,
  handleSetSecondFormComplete,
  isSecondFormComplete,
}) => {
  const {
    register,
    handleSubmit,
  ...
  useEffect(() => {
    if (
      user?.sex &&
      user.sex_orientation &&
      user.latino &&
      user.veteran &&
      user.disability &&
      user.authorized_to_work &&
      user.visa &&
      user.visa_type
    ) {
      handleSetSecondFormComplete();
      ...
      <form onSubmit={handleSubmit(onSubmit)} className={styles.formStyles}>
        ...
        >
          ...
          <Button
            ...
            variant="outlined"
            onClick={() => isSecondFormComplete && handleChangeForm(3)}
          >
            Next
          </Button>
